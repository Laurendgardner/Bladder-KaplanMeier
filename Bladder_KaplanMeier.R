...
bladder_data <- read.table(file = "bladder.tsv", header = TRUE, sep = "\t", stringsAsFactors = FALSE)
...
#Kaplan-Meier plot
survfit2(Surv(clinical_rfs_JD, NewRecurrenceOutcomeTable_num) ~ 1, data = bladder_data) %>% 
  ggsurvfit() +
  labs(
    x = "Months",
...
summary(survfit(Surv(clinical_rfs_JD, NewRecurrenceOutcomeTable_num) ~ 1, data = bladder_data), times = 12)

#table for 1-year recurrence free survival time estimate
survfit(Surv(clinical_rfs_JD, NewRecurrenceOutcomeTable_num) ~ 1, data = bladder_data) %>% 
  tbl_survfit(
    times = 12,
