...
survfit2(Surv(clinical_os_JD_num, NewRecurrenceOutcomeTable_num) ~ 1, data = bladder_data) %>% 
  ggsurvfit() +
  labs(
    x = "Weeks",
...
summary(survfit(Surv(clinical_os_JD_num, NewRecurrenceOutcomeTable_num) ~ 1, data = bladder_data), times = 52)

# Table for 1-year survival time estimate
survfit(Surv(clinical_os_JD_num, NewRecurrenceOutcomeTable_num) ~ 1, data = bladder_data) %>% 
  tbl_survfit(
    times = 52,
